...
const { STYLE_PRESETS, FONT_GENRES, SUPPORTING_FONT_GENRES, TITLE_EFFECTS, MOOD_PRESETS, HERO_PRESETS, BADGE_STYLES } = require("./visual-presets");
  ...
  return {
    ...
    frontCoverLockedZonesMm: {
      ...
      heroZone: {
        ...
        instruction: "hero utama mengikuti preset hero, jangan menabrak area judul atau footer"
      },
      authorZone: {
        localFromFrontTrim: { x: [15, 108], y: [256, 281] },
        globalCanvasX: [localToGlobal(15), localToGlobal(108)],
        instruction: "nama penulis kiri bawah"
      },
      authorSubtextZone: {
        localFromFrontTrim: { x: [15, 108], y: [282, 290] },
        globalCanvasX: [localToGlobal(15), localToGlobal(108)],
        instruction: "baris teks kecil di bawah nama penulis untuk keterangan penulis bila ada"
      },
      classZone: {
        localFromFrontTrim: { x: [125, 195], y: [246, 282] },
        globalCanvasX: [localToGlobal(125), localToGlobal(195)],
        instruction: "badge kelas kanan bawah, premium, mengikuti style badge yang dipilih"
      },
      footerTextZone: {
...
function buildCoverSpec(input) {
  ...
  const visualControls = {
    color_mode: clean(input.colorMode, 30) || "auto",
    ...
    hero_preset: {
      key: clean(input.heroPreset, 80) || "students_duo",
      label: (HERO_PRESETS[clean(input.heroPreset, 80)] || HERO_PRESETS.students_duo).label
    },
    font_genre: {
    ...
    author_font_genre: {
      key: clean(input.authorFontGenre, 80) || "sans_clean",
      label: (SUPPORTING_FONT_GENRES[clean(input.authorFontGenre, 80)] || SUPPORTING_FONT_GENRES.sans_clean).label
    },
    supporting_font_genre: {
      key: clean(input.supportingFontGenre, 80) || "sans_clean",
      label: (SUPPORTING_FONT_GENRES[clean(input.supportingFontGenre, 80)] || SUPPORTING_FONT_GENRES.sans_clean).label
    },
    title_text_effect: {
    ...
    badge_style: {
      key: clean(input.badgeStyle, 80) || "geometric_islamic",
      label: (BADGE_STYLES[clean(input.badgeStyle, 80)] || BADGE_STYLES.geometric_islamic).label
    },
    mood_preset: {
  ...
  const heroInfo = HERO_PRESETS[visualControls.hero_preset.key] || HERO_PRESETS.students_duo;
  const fontGenreInfo = FONT_GENRES[visualControls.font_genre.key] || FONT_GENRES.tegas_sans;
  const authorFontInfo = SUPPORTING_FONT_GENRES[visualControls.author_font_genre.key] || SUPPORTING_FONT_GENRES.sans_clean;
  const supportingFontInfo = SUPPORTING_FONT_GENRES[visualControls.supporting_font_genre.key] || SUPPORTING_FONT_GENRES.sans_clean;
  const titleEffectInfo = TITLE_EFFECTS[visualControls.title_text_effect.key] || TITLE_EFFECTS.clean_flat;
  const badgeInfo = BADGE_STYLES[visualControls.badge_style.key] || BADGE_STYLES.geometric_islamic;
  ...
  const exactText = [
    ...
    `Penulis: ${book.author}`,
    book.authorSubtext ? `Teks bawah penulis: ${book.authorSubtext}` : "Teks bawah penulis: kosongkan bila tidak ada",
    `Kelas/Jenjang: ${book.level} ${book.class}`.trim(),
    `Teks bawah kelas: ${visualControls.footer_text}`
  ].join(" | ");

  const colorDirection = visualControls.color_mode === "manual"
    ? `Gunakan primary color ${visualControls.primary_color} dan secondary/accent ${visualControls.secondary_color} sebagai acuan utama palet. Warna turunan boleh ditambahkan secara harmonis.`
    : `AI bebas memilih warna utama dan warna aksen yang paling sesuai dengan mata pelajaran, kelas, dan nuansa tema. Tidak harus mengikuti warna template default. Palet tetap harus harmonis, kontras, dan layak cetak.`;

  const heroAdditionalRule = heroInfo.supportsStudents
    ? `Jika hero menampilkan siswa/siswi, aturan pondok pesantren WAJIB dipatuhi: siswa laki-laki memakai songkok nasional hitam; siswi perempuan berhijab rapi; pakaian sopan; tidak bersentuhan; tidak bergandengan; tidak saling merangkul; bila ada dua tokoh siswa, posisikan kanan-kiri terpisah dengan jarak jelas demi menegaskan moral visual.`
    : `Hero boleh fokus pada objek, aktivitas, atau adegan materi. Jika siswa ditambahkan sebagai elemen pendukung, aturan pondok pesantren tetap wajib dipatuhi.`;
...
3. HERO: gunakan hero utama pada zona lokal x=24–186 mm dan y=120–246 mm. Preset hero yang dipilih: ${heroInfo.label}. Arah hero: ${heroInfo.direction}. ${heroAdditionalRule} Usia visual siswa, bila ada, sekitar ${age}, sesuai ${book.level} ${book.class}. Hero harus relevan dengan isi buku dan tidak menabrak area judul atau footer. ${visualControls.hero_note ? `Catatan hero tambahan: ${visualControls.hero_note}.` : ""}
4. PENULIS: tulis persis “${book.author}” di kiri bawah pada zona lokal x=15–108 mm dan y=256–281 mm.
5. TEKS BAWAH PENULIS: ${book.authorSubtext ? `tampilkan teks kecil persis “${book.authorSubtext}” tepat di bawah nama penulis pada zona lokal x=15–108 mm dan y=282–290 mm.` : "jika tidak ada teks bawah penulis, jangan menambahkan keterangan penulis buatan."}
6. KELAS: tampilkan badge kelas yang stylish dan premium di kanan bawah pada zona lokal x=125–195 mm dan y=246–282 mm. Style badge yang dipilih: ${badgeInfo.label}. Arah badge: ${badgeInfo.direction}. Tulis isi badge secara hierarkis dengan fokus utama pada “${book.class}”, serta tampilkan jenjang “${book.level}”.
7. FOOTER KELAS: tampilkan teks kecil persis “${visualControls.footer_text}” di area bawah/sekitar badge kelas, rapi, elegan, dan tidak lebih dominan dari badge.
8. Jangan pindahkan zona di atas. Variasi desain hanya boleh terjadi pada palet warna, suasana, motif mata pelajaran, komposisi ilustrasi di dalam zona hero, detail latar, dan treatment tipografi.
...
- Genre font penulis: ${authorFontInfo.label}. Arah tipografi: ${authorFontInfo.direction}.
- Genre font teks pendukung: ${supportingFontInfo.label}. Arah tipografi: ${supportingFontInfo.direction}.
...
Gunakan daftar isi sebagai sumber konsep visual. Pilih hanya 3–5 motif visual yang paling relevan dan representatif, lalu integrasikan secara natural ke lingkungan/aktivitas siswa atau scene materi. Jangan mengubah cover menjadi kolase ikon. Jangan menaruh puluhan simbol mengambang.
...
Instruksi custom di atas boleh memengaruhi suasana visual, ornamen, treatment tipografi, detail latar, dan detail hero, tetapi tidak boleh mengubah struktur layout lock.

QUALITY CONTROL SEBELUM FINAL
Periksa kembali bahwa: full wrap hanya satu gambar; ukuran dan punggung sesuai; cover depan berada di kanan; cover belakang di kiri; logo kanan atas; judul di tengah; penulis kiri bawah; badge kelas kanan bawah; semua teks minimal 15 mm dari trim; hero sesuai preset; jika ada siswa laki-laki maka bersongkok nasional; jika ada siswi maka berhijab; siswa-siswi tidak bersentuhan; ilustrasi relevan dengan daftar isi; tidak ada objek potong yang berisiko; visual bersih, profesional, tidak padat, dan konsisten sebagai seri buku sekolah.
`.trim();
  ...
  const negativePrompt = [
    ...
    "siswa laki-laki tanpa songkok nasional saat hero siswa digunakan",
    "siswi perempuan tanpa hijab",
    "siswa dan siswi saling bersentuhan",
    "bergandengan tangan",
    "saling merangkul",
